...
export const Stagger3D = ({images, order = [4, 6, 9, 0, 1, 5, 11, 2, 3, 13, 7, 8, 10, 12]}:grid) => {
  ...
  return (
   <div ref={containerRef} className='flex h-[200vh] md:h-[300vh] relative'>
    ...
    <div className='sticky top-30 md:top-20 h-fit md:h-screen w-full'>
        ...
        className='w-full grid grid-cols-3 md:grid-cols-4 gap-2 relative'>
            ...
            className='absolute top-0 w-full inset-0 bg-black/40 flex items-center p-2 justify-center'>
                <h1 className='text-6xl w-[70%] text-center'>
                    Nustalgia. Into the Abyst
                </h1>
            </div>
        </div>
    </div>
...
const GridCard = ({scrollYProgress, index, image, xoffset, yoffset}:gridcard) => {
