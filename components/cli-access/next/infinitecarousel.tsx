...
export const InfiniteCarousel = ({autoplay = false, playDuration = 2, ImageCards=[""], className, cardClassName}:carouselProps) => {
    const [current, setcurrent] = useState(0)
    const length = ImageCards?.length 

    const prevIndex = (i:number) => (i - 1 + length) % length
    const nextIndex = (i:number) => (i + 1) % length
  ...
  return (
    <div style={{perspective: 500, transformStyle: "preserve-3d"}} className={cn('p-3 md:max-w-80 w-[90%] h-[400px] flex relative', className)}>
        {
